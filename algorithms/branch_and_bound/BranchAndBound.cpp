...
void BranchAndBound::branch_n_bound(vector<int> &tab, int level, int lower_bound, int distance, int &min_distance) {
    if (level == dimension) {
        ...
        for (int i = level; i < dimension; i++) {
            ...
            if (level == 1) lower_bound -= ceil((min_distance_from(tab[0]) + min_distance_from(tab[i])) / 2.0);
                ...
                lower_bound -= ceil((second_distance_from(tab[level - 1]) + second_distance_from(tab[0])) / 2.0);
...
int BranchAndBound::min_distance_from(int vertex) {
    int min = INT_MAX;
...
int BranchAndBound::second_distance_from(int vertex) {
